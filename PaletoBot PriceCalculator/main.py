"""Paleto Tuners Bot - GTA RP mechanic price calculator & invoicing.

Admins deploy an interactive dashboard (category -> services -> quantities ->
client type) that calculates totals, applies LSPD/EMS discounts, shows the
customer a receipt, and logs every invoice to a Discord webhook.

Prices live in prices.json: {"Service Name": <int price> or null}. A null /
non-int value marks the service as unavailable.
"""
...
log = logging.getLogger("PaletoBot")
...
prices = {
    k: {"price": v if isinstance(v, int) else 0, "available": isinstance(v, int)}
...
# Webhook logging
async def send_webhook(url: str, embed: discord.Embed) -> None:
...
class QuantityModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            "✅ Quantities updated! Click **Checkout** to finish.", ephemeral=True
        )
    ...
    async def select_client(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if self.webhook_url:
            log_embed = discord.Embed(
                title="📋 New Invoice", color=0x00B894, timestamp=interaction.created_at
            )
...
class JobSelectView(discord.ui.View):
    def __init__(self, category, webhook_url):
        ...
        self.webhook_url = webhook_url
        self.session_data = {}  # {JobName: Quantity}
        self.selected_jobs = []
        ...
        if not options:
            return  # No available services in this category.
    ...
    async def edit_qty(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self.selected_jobs:
            return await interaction.response.send_message(
                "⚠️ Select at least one job first!", ephemeral=True
            )
        if len(self.selected_jobs) > 5:
            return await interaction.response.send_message(
                "⚠️ You can only edit 5 items at a time due to Discord limits.", ephemeral=True
            )
        await interaction.response.send_modal(
            QuantityModal(self.selected_jobs, self.session_data, interaction)
        )
...
class CategoryView(discord.ui.View):
    ...
    async def select_category(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        await interaction.response.send_message(
            f"**{category}** - Select items:", view=view, ephemeral=True
        )
...
@app_commands.describe(
    menu_channel="Where to post prices",
    job_channel="Where to post the dashboard",
    webhook="Log Webhook URL",
)
@app_commands.checks.has_permissions(administrator=True)
async def setup_paleto(
    interaction: discord.Interaction,
    menu_channel: discord.TextChannel,
    job_channel: discord.TextChannel,
    webhook: str,
) -> None:
...
async def on_ready() -> None:
    log.info("Logged in as %s (Paleto Bot)", bot.user)
